...
def get_possible_decisions(decisions):
    ...
    if len(decisions)==0:
        return ["bet", "check"]
    list_of_actions = decisions.split("-")
    if list_of_actions[-1]=="bet":
        return ["call", "fold"]
    elif list_of_actions[-1]=="check":
        return ["bet", "check"]
...
def profit(decisions, players_cards):
    ...
    '''
    list_of_actions = decisions.split("-")
    player_on_move = len(list_of_actions)%2
    
    if list_of_actions[-1]=="fold":
        return 1

    else:
        stake = 1 #case without betting
        if "bet" in list_of_actions:
